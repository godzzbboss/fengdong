...
if __name__ == "__main__":
    ...
    for epoch in range(EPOCH):
        for batch_idx, sample_batched in enumerate(dataloader):
            sample_batched = sample_batched.to(torch.float32)
            batch_X = sample_batched[:,:,:3]
            batch_y = sample_batched[:,:,[3]]

            out = net(batch_X) # 前向传播
            ...
            #     # plt.scatter(X[:,0].detach().numpy(), out.detach().numpy(), c="red")
